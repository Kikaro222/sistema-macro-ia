...
def enviar_telegram(token, chat_id, texto, tentativas=2):
    ...
    for tentativa in range(tentativas):
        try:
            ...
            if r.ok:
                return True, "OK"
            erro = f"HTTP {r.status_code}: {r.text[:100]}"
        except requests.exceptions.Timeout:
            erro = "Timeout (servidor demorou)"
...
api_key = st.sidebar.text_input("Chave FRED:", type="password", value=segredo("FRED_API_KEY"))

st.sidebar.markdown("### 📱 Telegram")
tg_token = st.sidebar.text_input("Token do Bot:", type="password", value=segredo("TELEGRAM_TOKEN"))
tg_chat = st.sidebar.text_input("Seu Chat ID:", value=segredo("TELEGRAM_CHAT_ID"))
...
if tg_token and tg_chat:
    if st.sidebar.button("📨 Testar Telegram"):
        ok, erro = enviar_telegram(tg_token, tg_chat, "✅ Sistema Macro IA conectado com sucesso!")
        if ok:
...
st.markdown("---")
st.markdown("<div style='text-align:center; color:#888;'>Sistema Macro IA v4.3 — EUA (FRED) • "
